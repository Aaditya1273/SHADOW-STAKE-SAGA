...
export class Enemy {
  ...
  constructor(
    ...
    // Scale all enemies consistently
    const largeEnemies = ['shadow-beast', 'elemental-wraith', 'necromancer'];
    if (largeEnemies.includes(this.enemyType.key)) {
      this.sprite.setScale(1.5); // 22-39px -> 33-58px per frame
    } else {
      this.sprite.setScale(2); // 16px -> 32px for pixel art
    }
  ...
  public onHitByPlayer(scene: Phaser.Scene): void {
    const damage = 30.075; // Increased by 0.25% from original 30
  ...
  destroy() {
    ...
    gameState.trackEnemyKill(this.enemyType.key);
